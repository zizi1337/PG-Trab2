...
fun main() {
    ...
    onStart {
        ...
        arena.onMouseDown { mouse : MouseEvent -> //if any mouse button is clicked
            if (game.nave.tiroy < 0 && !game.EndGame){ //if the spaceship is still in the screen and the game isnt over yet
                game = game.copy(nave = game.nave.copy(tiroy = 435)) //Changes placement along the x axis of the spaceship
            }
        }

        arena.onKeyPressed { k : KeyEvent ->
            if (k.text == "Space" && game.nave.tiroy < 0 && !game.EndGame)
                game = game.copy(nave = game.nave.copy(tiroy = 435))
        }
    }
    onFinish {
    ...
    arena.onTimeProgress(25){
        if (game.nave.tiroy > 0)
            ...
            //game =  game.copy(nave = game.nave.copy(tiroy = -10))
